# ══════════════════════════════════════════════════════════════════════════════
#  COMPLETE app.py  —  fixes login/register/session flow
# ══════════════════════════════════════════════════════════════════════════════
...
# Make session cookie work across restarts
...
def init_db():
    try:
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS songs(
            ...
            has_audio TINYINT(1) DEFAULT 0, has_video TINYINT(1) DEFAULT 0,
...
def serve_audio(filename): return send_from_directory(UPLOAD_AUDIO, filename)

@app.route("/uploads/video/<path:filename>")
def serve_video(filename):  return send_from_directory(UPLOAD_VIDEO, filename)

# ══════════════════════════════════════════════════════════════════════════════
#  PAGE ROUTES  —  Correct flow:
#  /  →  home.html  (public, always shown first)
...
def register():
    try:
        ...
        # Check duplicate email — clear message
        ...
        # Set session immediately after register
...
def get_songs():
    try:
        ...
        for s in songs:
            if s.get("has_audio") and s.get("audio_filename"): s["audio_url"]="/uploads/audio/"+s["audio_filename"]
            if s.get("has_video") and s.get("video_filename"): s["video_url"]="/uploads/video/"+s["video_filename"]
...
def upload_song():
    try:
        ...
        if not title or not artist or not mood:
            return jsonify({"ok":False,"error":"Title, artist and mood required."}),400
        genre=request.form.get("genre","Unknown"); tempo=float(request.form.get("tempo",120))
        energy=float(request.form.get("energy",0.5)); valence=float(request.form.get("valence",0.5))
        danceability=float(request.form.get("danceability",0.5))
        manual_url=request.form.get("audio_url","").strip()
        audio_filename=""; video_filename=""; has_audio=0; has_video=0; fsz=0.0

        af=request.files.get("audio_file")
        if af and af.filename:
            if not allowed_audio(af.filename): return jsonify({"ok":False,"error":"Invalid audio format."}),400
            safe=secure_filename(af.filename); base,ext=os.path.splitext(safe)
            unique=f"{base}_{random.randint(10000,99999)}{ext}"; path=os.path.join(UPLOAD_AUDIO,unique)
            af.save(path); sz=os.path.getsize(path)/1024/1024
            if sz>50: os.remove(path); return jsonify({"ok":False,"error":"Audio too large (max 50MB)."}),400
            audio_filename=unique; has_audio=1; fsz=round(sz,2)

        vf=request.files.get("video_file")
        if vf and vf.filename:
            if not allowed_video(vf.filename): return jsonify({"ok":False,"error":"Invalid video format."}),400
            safe=secure_filename(vf.filename); base,ext=os.path.splitext(safe)
            unique=f"{base}_{random.randint(10000,99999)}{ext}"; path=os.path.join(UPLOAD_VIDEO,unique)
            vf.save(path); sz=os.path.getsize(path)/1024/1024
            if sz>200: os.remove(path); return jsonify({"ok":False,"error":"Video too large (max 200MB)."}),400
            video_filename=unique; has_video=1
        ...
        return jsonify({"ok":True,"id":new_id,"has_audio":bool(has_audio),"has_video":bool(has_video)})
...
def delete_song(sid):
    try:
        db=get_db(); cur=db.cursor(dictionary=True)
        cur.execute("SELECT * FROM songs WHERE id=%s",(sid,)); s=cur.fetchone()
        if s:
            for f,d in [(s.get("audio_filename"),UPLOAD_AUDIO),(s.get("video_filename"),UPLOAD_VIDEO)]:
                if f:
                    p=os.path.join(d,f)
...
def analyze_route():
    try:
        ...
        return jsonify({"mood":final_mood,"predicted_mood":song["mood"],"confidence":conf,
                        "song":add_urls(song),"recommendations":[add_urls(r) for r in recs],
...
def get_history():
    try:
        sid=request.args.get("session_id","default"); db=get_db(); cur=db.cursor(dictionary=True)
        cur.execute("""SELECT s.*,lh.played_at FROM listening_history lh JOIN songs s ON lh.song_id=s.id
            WHERE lh.session_id=%s ORDER BY lh.played_at DESC LIMIT 20""",(sid,))
        rows=cur.fetchall(); db.close()
        for r in rows:
            if r.get("played_at"): r["played_at"]=str(r["played_at"])
            if r.get("has_audio") and r.get("audio_filename"): r["audio_url"]="/uploads/audio/"+r["audio_filename"]
        return jsonify(rows)
    except Exception as e: return jsonify({"error":str(e)}),500
...
def admin_stats():
    try:
        db=get_db(); cur=db.cursor(dictionary=True)
        cur.execute("SELECT COUNT(*) AS t FROM songs"); total=cur.fetchone()["t"]
        cur.execute("SELECT mood,COUNT(*) AS c FROM songs GROUP BY mood"); mood_dist=cur.fetchall()
        cur.execute("SELECT COUNT(DISTINCT session_id) AS s FROM listening_history"); sessions=cur.fetchone()["s"]
        cur.execute("SELECT COUNT(*) AS p FROM listening_history"); plays=cur.fetchone()["p"]
        cur.execute("SELECT COUNT(*) AS u FROM users"); users=cur.fetchone()["u"]
        db.close()
        return jsonify({"total_songs":total,"mood_dist":mood_dist,"sessions":sessions,
                        "total_plays":plays,"total_users":users})
    except Exception as e: return jsonify({"error":str(e)}),500
